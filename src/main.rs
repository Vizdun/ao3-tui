...
use cursive::{
    ...
    view::SizeConstraint,
    views::{
        Checkbox, Dialog, EditView, EnableableView, FixedLayout, LinearLayout, ListChild, ListView,
    ...
    Dialog::new()
        ...
        .button("search", move |s| {
            ...
            let warnings: HashSet<Warning> = Warning::iter()
                ...
                .collect();
            let categories: HashSet<Category> = Category::iter()
                .filter(|x| {
            ...
            let mut sq = SearchQuery::builder()
                ...
                .tags(tags)
                .hits(parse_range(hits.to_string()))
                .kudos(parse_range(kudos.to_string()))
                .comments(parse_range(comments.to_string()))
                .bookmarks(parse_range(bookmarks.to_string()))
                .sort_by(Rc::<SortBy>::unwrap_or_clone(sort_by))
                .sort_direction(Rc::<SortDirection>::unwrap_or_clone(sort_direction))
                .build();

            s.pop_layer();
            s.add_layer(SearchResults { sq: sq.clone() }.with_name("sq"));
            s.add_layer(search_result_ui(&mut sq));
        })
}
...
fn search_result_ui(sq: &mut SearchQuery) -> SelectView<u32> {
    // TODO: crashes is sq is without any props for some reason
    sq.take(10)
        .fold(SelectView::new(), |l, x| l.item(&x.title, x.id))
        .on_submit(|s, item| {
            let work = Work::from_id(*item);
            s.pop_layer();
            s.add_layer(
                App {
                    chapter: 0,
            ...
            s.add_layer(render_work(&work, 0))
        })
}
...
fn main() -> Result<(), io::Error> {
    ...
    siv.add_global_callback(Event::Key(Key::Backspace), |s| {
        if let Some(()) = s.call_on_name("app", |_: &mut App| {}) {
            s.pop_layer();
            s.pop_layer();

            let mut sq = s
                .call_on_name("sq", |sq: &mut SearchResults| sq.sq.clone())
                .unwrap();

            s.add_layer(search_result_ui(&mut sq));
        } else if let Some(()) = s.call_on_name("sq", |_: &mut SearchResults| {}) {
            s.pop_layer();
            s.pop_layer();

            s.add_layer(search_query_ui());
        }
    });
